...
// more specific first
    ...
    interface Processor<T> {
        T process(Exception ex, ErrorType type, String msg, WebRequest request);
    }

    protected <T> T processException(@NonNull Exception ex, WebRequest request, Processor<T> processor) {
        String path = null;
        if (request instanceof ServletWebRequest servletWebRequest) {
            path = servletWebRequest.getRequest().getRequestURI();
        }
        return processException(ex, request, path, processor);
    }

    protected <T> T processException(@NonNull Exception ex, @Nullable WebRequest request, String path, Processor<T> processor) {
